...
import { Bot, User, AlertTriangle, CheckCircle, Clock, Mic, MicOff } from 'lucide-react';
...
export function VoiceAIConsultation({ onNext, language }: VoiceAIConsultationProps) {
  ...
  return (
    <div className="flex flex-col items-center w-full h-[80vh]">
      ...
      <div className="text-center mb-4">
        <h2 className="text-2xl font-bold text-[var(--color-text-primary)]">{isHi ? 'AI वॉयस परामर्श' : 'AI Voice Consultation'}</h2>
        <p className="text-sm text-[var(--color-text-secondary)]">{isHi ? 'बोलना शुरू करने के लिए माइक बटन दबाएं' : 'Tap the mic button to start speaking'}</p>
      </div>
      ...
      <div className="bg-[var(--color-surface)] border-2 border-[var(--color-border)] rounded-xl p-4 w-full max-w-3xl flex-1 flex flex-col overflow-hidden shadow-sm">
        ...
        <div className="flex flex-col items-center gap-3">
          ...
          <span className={`text-sm ${isListening ? 'text-[var(--color-danger)]' : 'text-[var(--color-text-secondary)]'}`}>
            {isListening ? (isHi ? 'सुन रहा है...' : 'Listening...') : (isHi ? 'बोलने के लिए टैप करें' : 'Tap to speak')}
          </span>
        </div>
      </div>
