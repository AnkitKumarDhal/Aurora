  ...
  if (showInput) {
    ...
    return (
      <div className="flex flex-col items-center justify-center w-full h-screen overflow-hidden bg-[var(--color-bg)]">
        ...
        <button
          onClick={handleBack}
          className="absolute top-8 left-8 p-2 rounded-full hover:bg-[var(--color-surface-alt)] transition-colors"
        >
        ...
        <div className="grid grid-cols-3 gap-3 mb-6">
          ...
          <motion.button
            ...
            whileTap={{ scale: 0.95 }}
            onClick={handleClear}
            className="w-24 h-16 rounded-xl bg-[var(--color-surface)] border-2 border-[var(--color-danger)] text-base font-semibold text-[var(--color-danger)] hover:bg-[var(--color-danger)] hover:text-white transition-all shadow-md"
          >
            Clear
          </motion.button>
          <motion.button
          ...
          <motion.button
            ...
            className="w-24 h-16 rounded-xl bg-[var(--color-surface)] border-2 border-[var(--color-border)] text-xl font-semibold text-[var(--color-text-secondary)] hover:bg-[var(--color-warning)] hover:text-white transition-all shadow-md"
          >
            ←
          </motion.button>
        </div>
        ...
        <div className="flex gap-4">
          <Button
            ...
            className={`px-6 py-4 text-lg rounded-xl transition-all ${useBiometric ? "bg-[var(--color-primary)] text-white shadow-lg" : "bg-transparent border-2 border-[var(--color-border)] text-[var(--color-text-secondary)]"}`}
          >
            <Fingerprint className="h-5 w-5 mr-2 inline" />{" "}
            {isHi ? "बायोमेट्रिक" : "Biometric"}
          </Button>
          <Button
            onClick={handleContinue}
            disabled={idNumber.length < maxLength && !useBiometric}
            className="px-8 py-4 text-lg rounded-xl bg-[var(--color-primary-dark)] hover:bg-[var(--color-text-primary)] text-white disabled:opacity-40 transition-all shadow-lg"
          >
    ...
    <div className="flex flex-col items-center justify-center w-full h-screen overflow-hidden">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-8 w-full max-w-4xl px-4">
        <motion.button
          ...
          className={`h-64 rounded-2xl border-2 transition-all duration-200 flex flex-col items-center justify-center gap-6 ${selectedId === "abha" ? "border-[var(--color-primary)] bg-[var(--color-primary-tint)] shadow-lg" : "border-[var(--color-border)] bg-[var(--color-surface)] hover:border-[var(--color-primary)]"}`}
        >
          <User
            className={`h-16 w-16 ${selectedId === "abha" ? "text-[var(--color-primary-dark)]" : "text-[var(--color-text-secondary)]"}`}
          />
        ...
        <motion.button
          ...
          className={`h-64 rounded-2xl border-2 transition-all duration-200 flex flex-col items-center justify-center gap-6 ${selectedId === "aadhaar" ? "border-[var(--color-primary)] bg-[var(--color-primary-tint)] shadow-lg" : "border-[var(--color-border)] bg-[var(--color-surface)] hover:border-[var(--color-primary)]"}`}
        >
          <Fingerprint
            className={`h-16 w-16 ${selectedId === "aadhaar" ? "text-[var(--color-primary-dark)]" : "text-[var(--color-text-secondary)]"}`}
          />
          <div className="text-3xl font-semibold text-[var(--color-text-primary)]">
            {isHi ? "आधार" : "Aadhaar"}
          </div>
