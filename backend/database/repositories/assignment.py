...
class AssignmentRepository(BaseRepository[DoctorAssignmentDocument]):
    ...
    async def get_session_assignment_history(self, session_id: str) -> DoctorAssignmentDocument | None:
        return await self.get_one({
            "session_id": session_id,
        },
        )
    ...
    async def get_doctor_assignments(self, doctor_id: str) -> list[DoctorAssignmentDocument]:
        ...
        return [self.model.from_mongo(document) async for document in cursor]
