...
class WorkflowService:
    def __init__(
        ...
        promotion_workflow: PromotionWorkflowService,
    ) -> None:
        ...
        self.promotion_workflow = (
            promotion_workflow
        )

    async def queue_session_from_triage(
        self,
        session_id: str,
    ) -> QueueEntry:
        session = (
        ...
        if triage.urgency_level is None:
            raise ValueError(
                "Triage result has no urgency level",
            )

        if triage.priority_score is None:
            raise ValueError(
                "Triage result has no priority score",
            )
        ...
        return await self.queue_session(
            entry,
        )
    ...
    async def queue_session(
        ...
        if (
            ...
            raise ValueError(
                "Queue entry does not belong to session",
            )
        ...
        if (
            ...
            raise ValueError(
                "Queue entry department does not match session",
            )
    ...
    async def queue_and_assign(
        ...
        await self.promotion_workflow.evaluate_entry(
            existing_entry,
        )
    ...
    async def assign_patient(
        ...
        if (
            ...
            raise ValueError(
                "Session must be queued before assignment",
            )
        ...
        if (
            ...
            raise ValueError(
                "Queue entry must be waiting before assignment",
            )

        if entry.doctor_id is not None:
            raise ValueError(
                "Queue entry is already assigned",
            )

        assignment = (
            await self.assignment_scheduler
            .assign(entry)
        )
    ...
    async def call_patient(
        ...
        if called_entry is None:
            raise ValueError(
                "Queue entry could not be called",
            )
    ...
    async def start_consultation(
        ...
        if (
            ...
            raise ValueError(
                "Session must be called before consultation",
            )
        ...
        if (
            ...
            raise ValueError(
                "Queue entry must be called before consultation",
            )
        ...
        if consultation_entry is None:
            raise ValueError(
                "Queue entry could not start consultation",
            )
    ...
    async def complete_consultation(
        ...
        if (
            ...
            raise ValueError(
                "Session is not in consultation",
            )
        ...
        if (
            ...
            raise ValueError(
                "Queue entry is not in consultation",
            )

        assignment = (
            await self.assignment_service
        ...
        if released_assignment is None:
            raise ValueError(
                "Doctor assignment could not be released",
            )
        ...
        await self.promotion_workflow.evaluate_department(
            session.department_id,
        )
