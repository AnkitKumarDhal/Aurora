...
export function WaitingScreen({ onReset, language }: WaitingScreenProps) {
  ...
  return (
    <div className="flex flex-col items-center justify-center w-full h-[80vh] relative overflow-hidden">
      ...
      >
        {isHi ? 'आपका पंजीकरण पूरा हो गया है। डॉक्टर जल्द ही आपको बुलाएंगे।' : 'Your registration is complete. The doctor will call you shortly.'}
      </motion.p>

      {/* Circular Timer */}
      <div className="relative flex items-center justify-center mb-6">
        <svg className="transform -rotate-90 w-32 h-32">
          <circle cx="64" cy="64" r={radius} stroke="var(--color-border)" strokeWidth="8" fill="transparent" />
          <circle
      ...
      <p className="text-lg text-[var(--color-text-secondary)] mb-6">
        {isHi ? `अगले रोगी के लिए ${timeLeft} सेकंड में रीसेट हो रहा है` : `Resetting for next patient in ${timeLeft} seconds`}
      </p>

      <Button onClick={onReset} className="px-12 py-6 text-xl rounded-xl bg-[var(--color-primary-dark)] hover:bg-[var(--color-text-primary)] text-white transition-all shadow-lg min-w-[280px] flex items-center gap-3">
